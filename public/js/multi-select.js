...
function MultiselectDropdown(options, width = '680px') {
	...
	document.querySelectorAll("select[multiple]").forEach((el, k) => {
		...
		let div = newEl('div', { class: 'multiselect-dropdown', style: { width: config.width ?? el.clientWidth + 'px', padding: config.style?.padding ?? '' } });
		...
		el.loadOptions = () => {
			...
			if (el.attributes['multiselect-select-all']?.value == 'true') {
				...
				let ic = newEl('input', { class: 'checkbox', type: 'checkbox' });
				op.appendChild(ic);
			...
			Array.from(el.options).map(o => {
				...
				let ic = newEl('input', { class: 'checkbox', type: 'checkbox', checked: o.selected });
				op.appendChild(ic);
